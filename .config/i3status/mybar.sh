...
mycrypto() {
  ...
  echo -n "\"full_text\":\" $(/home/rusito/.config/i3status/crypto.py) \","
  echo -n "\"color\":\"#0a0a0a\","
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $(/home/rusito/.config/i3status/disk.py) GB\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
memory() {
  ...
  echo -n "\"full_text\":\"  $(/home/rusito/.config/i3status/memory.py) \","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $(/home/rusito/.config/i3status/cpu.py)% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $(/home/rusito/.config/i3status/meteo.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
do
  ...
  if [[ $line == *"name"*"id_vpn"* ]]; then
    xterm -e /home/rusito/.config/i3status/click_vpn.sh &
    
   # WiFi menu
  elif [[ $line == *"name"*"ip_local"* ]]; then
    alacritty -e /home/rusito/.config/i3status/op.sh &
  ...
  # TIME
    ...
    alacritty -e /home/rusito/.config/i3status/click_time.sh &
  # METEO
  elif [[ $line == *"name"*"id_meteo"* ]]; then
    xdg-open https://openweathermap.org/city/2515270 > /dev/null &

  # CRYPTO
  elif [[ $line == *"name"*"id_crypto"* ]]; then
    xdg-open https://coinmarketcap.com/ > /dev/null &

  # VOLUME
  elif [[ $line == *"name"*"id_volume"* ]]; then
    xterm -e pactl set-sink-mute @DEFAULT_SINK@ toggle &
 ...
 #   i3-nagbar -t warning -m 'Log out ?' -b 'yes' 'i3-msg exit' > /dev/null &
    python3 /home/rusito/.config/i3status/logout.py &
